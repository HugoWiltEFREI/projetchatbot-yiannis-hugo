...
def associationTxtPrenomPres(nom):
    ...
    for indice in range(len(ListeNomsPres)):  # On parcours la liste
        if ListeNomsPres[indice][0] == nom:  # On compare si le premier élément (indice 0) de chaques sous-tableaux est égal au nom
            print("Le prénom du Président", nom, "est",
                  ListeNomsPres[indice][1])  # Si oui, on affiche le prénom et le nom
            trouve = True
    if not trouve:
